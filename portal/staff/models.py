...
def rename_upload_file(instance, filename):
    ...
    # Get the field name based on the upload instance
    field_name = None
    for field in instance._meta.fields:
        if hasattr(field, 'upload_to') and callable(field.upload_to):
            if field.upload_to == rename_upload_file:
                field_name = field.name

    # Create a new filename using the operation code and field name
    new_filename = "{0}-{1}.{2}".format(instance.fuel_order.operation_code, field_name, ext)

    # Return the complete path
    if field_name == 'tractor_pic':
        return 'static/documents/tractor/{}'.format(new_filename)
    elif field_name == 'backpack_pic':
        return 'static/documents/backpack/{}'.format(new_filename)
    elif field_name == 'chamber_pic':
        return 'static/documents/chamber/{}'.format(new_filename)
    else:
        return os.path.join('static/documents/', new_filename)
...
class Refuelings(models.Model):
    ...
    tractor_pic = models.ImageField(upload_to=rename_upload_file, null=True, blank=True)
    backpack_pic = models.ImageField(upload_to=rename_upload_file, null=True, blank=True)
    chamber_pic = models.ImageField(upload_to=rename_upload_file, null=True, blank=True)
